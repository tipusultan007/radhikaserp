@extends('layouts.vertical', ['page_title' => 'Edit Supplier Payment', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('supplier-payables.update', $journal->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <select name="supplier_id" class="form-select select2" data-toggle="select2" required>
                                ...
                                @foreach($suppliersList as $s)
                                    <option value="{{ $s->id }}" {{ $journal->reference_id == $s->id ? 'selected' : '' }}>
                                        {{ $s->name }} (Bal: ${{ number_format($s->total_payable, 0) }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="number" step="1" name="amount" class="form-control" value="{{ $amount }}" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('supplier-payables.index') }}" class="btn btn-light">Cancel</a>
