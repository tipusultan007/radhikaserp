@extends('layouts.vertical', ['page_title' => 'Edit Purchase Shipment', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    @php
        ...
        $selectedType = old('purchase_type', $purchase->purchase_type ?? 'imported');
        $existingBreakdown = old('cost_breakdown', $purchase->cost_breakdown ?? []);
    @endphp
         ...
         <div class="row">
            <div class="col-12">
                <div class="page-title-box justify-content-between d-flex align-items-md-center flex-md-row flex-column">
                    <h4 class="page-title">Edit Purchase Shipment #{{ $purchase->purchase_no }}</h4>
                         ...
                         <div class="card-body">
                             ...
                             @if(!empty($hasConsumedStock))
                                 <div class="alert alert-warning border-0 shadow-sm mb-3">
                                     <i class="ri-alert-line me-1"></i> Warning: Stock from this purchase batch has already been consumed in repackaging or sales. Modifying this purchase may affect inventory consistency.
                                 </div>
                             @endif
                                 ...
                                 <div class="col-md-4">
                                     <div class="mb-3">
                                         ...
                                         <div class="input-group">
                                            ...
                                            <input type="text" name="date" class="form-control flatpickr-date" value="{{ old('date', $purchase->date->format('Y-m-d')) }}" required>
                                         </div>
                                     </div>
                                 </div>
                                 <div class="col-md-4">
                                     ...
                                     <div class="mb-3">
                                         ...
                                         <select name="warehouse_id" class="form-select select2" required>
                                             ...
                                             @foreach($warehouses as $wh)
                                                 <option value="{{ $wh->id }}" {{ old('warehouse_id', $purchase->warehouse_id) == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                                             @endforeach
                                         </select>
                                     </div>
                                 </div>
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-hover table-bordered mb-0" id="itemsTable">
                                    ...
                                    @php
                                        $oldItems = old('items');
                                        $displayItems = ($oldItems && is_array($oldItems) && count($oldItems) > 0) ? $oldItems : $purchase->items;
                                    @endphp
                                            ...
                                            <tr class="item-row">
                                                ...
                                                <td>
                                                    <div class="input-group input-group-sm">
                                                        <input type="number" step="0.001" min="0.001" name="items[{{ $index }}][qty]" class="form-control item-qty" placeholder="0.00" value="{{ $qtyVal }}" required>
                                                        <span class="input-group-text unit-addon">{{ $unitName }}</span>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="input-group input-group-sm">
                                                ...
                                                <td class="text-center align-middle">
                                                    <button type="button" class="btn btn-sm btn-soft-danger remove-row" {{ count($displayItems) == 1 ? 'disabled' : '' }}>
                 ...
                 <div class="col-lg-12">
                     <div class="card shadow-sm border-0">
                         ...
                         <div id="local-cost-container" class="card-body" style="{{ $selectedType == 'local' ? '' : 'display: none;' }}">
                             <div class="row">
                                 <div class="col-md-4">
                                     ...
                                     <div class="input-group">
                                         <span class="input-group-text">$</span>
                                         <input type="number" step="any" min="0" name="delivery_cost" id="delivery_cost" class="form-control" placeholder="0.00" value="{{ old('delivery_cost', $purchase->delivery_cost ?? '0') }}">
                                     </div>
                                 </div>
                             </div>
                         </div>
                                 ...
                                 <table class="table table-bordered table-hover mb-0 align-middle">
                                     ...
                                     <tbody>
                                         @foreach($landedDescriptions as $idx => $desc)
                                             @php
                                                 $savedRow = is_array($existingBreakdown) ? collect($existingBreakdown)->firstWhere('description', $desc) : null;
                                                 if (!$savedRow && is_array($existingBreakdown) && isset($existingBreakdown[$idx])) {
                                                     $savedRow = $existingBreakdown[$idx];
                                                 }
                                                 $val = $savedRow['amount'] ?? ($savedRow['bd_cost'] ?? '');
                                             @endphp
                         ...
                         <div class="card-footer bg-white border-top text-end py-3">
                             <a href="{{ route('purchases.show', $purchase->id) }}" class="btn btn-light me-2"><i class="ri-close-line"></i> Cancel</a>
...
$(document).ready(function() {
    ...
    // Initial calculate to ensure everything matches
    calculateTotals();
