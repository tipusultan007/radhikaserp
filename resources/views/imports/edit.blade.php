@extends('layouts.vertical', ['page_title' => 'Edit Import Shipment', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    <div class="container-fluid">
         <div class="row">
            <div class="col-12">
                <div class="page-title-box justify-content-between d-flex align-items-md-center flex-md-row flex-column">
                    <h4 class="page-title">Edit Import: {{ $import->import_no }}</h4>
         ...
         <form action="{{ route('imports.update', $import->id) }}" method="POST" id="importForm">
             @csrf
             @method('PUT')
             <div class="row">
                 <div class="col-lg-12">
                                 ...
                                 <div class="col-md-4">
                                     <div class="mb-3">
                                         ...
                                         <div class="input-group">
                                            ...
                                            <input type="text" name="date" class="form-control flatpickr-date" value="{{ old('date', $import->date->format('Y-m-d')) }}" required>
                                         </div>
                                     </div>
                                 </div>
                                 <div class="col-md-4">
                                     ...
                                     <div class="mb-3">
                                         ...
                                         <select name="warehouse_id" class="form-select select2" required>
                                             ...
                                             @foreach($warehouses as $wh)
                                                 <option value="{{ $wh->id }}" {{ $import->warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                                             @endforeach
                                         </select>
                                     </div>
                                 </div>
                                ...
                                <table class="table table-hover table-bordered mb-0" id="itemsTable">
                                            ...
                                            <td>
                                                <select name="items[{{ $index }}][product_id]" class="form-select product-select" required>
                                                    ...
                                                    @foreach($products as $product)
                                                        <option value="{{ $product->id }}" data-unit="{{ $product->unit ? $product->unit->short_name : 'Unit' }}" {{ $item->product_id == $product->id ? 'selected' : '' }}>{{ $product->name }} ({{ $product->unit ? $product->unit->short_name : 'Unit' }})</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td>
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <span class="input-group-text unit-addon">{{ $item->product && $item->product->unit ? $item->product->unit->short_name : 'Unit' }}</span>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text">$</span>
                                                    <input type="number" step="any" min="0.01" name="items[{{ $index }}][unit_cost]" class="form-control item-cost" placeholder="0.00" value="{{ $item->unit_cost }}" required>
                                                </div>
                                            </td>
                                            <td class="text-end align-middle">
                                                <span class="fw-bold row-subtotal">${{ number_format($item->qty * $item->unit_cost, 2, '.', '') }}</span>
                                            </td>
                                            <td class="text-center align-middle">
                                                <button type="button" class="btn btn-sm btn-soft-danger remove-row" {{ count($import->items) == 1 ? 'disabled' : '' }}>
                                    ...
                                    <tfoot>
                                        <tr class="table-light">
                                            ...
                                            <td class="text-end fw-bold text-success fs-4" id="grandTotal">${{ number_format($import->total_cost, 2, '.', '') }}</td>
                                            <td></td>
                                        </tr>
                                    </tfoot>
                                </table>
...
$(document).ready(function() {
    ...
    $('#addRowBtn').click(function() {
        // Just copy options from the first select
        let options = $('.product-select:first').html(); 
        
        // Ensure "selected" is removed from the cloned options string
